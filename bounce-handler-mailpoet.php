...
function bhm_load_text_domain() {

	load_plugin_textdomain( 'bounce-handler-mailpoet', false,
		dirname( plugin_basename( __FILE__ ) ) . '/languages' );
}
add_action( 'plugins_loaded', 'bhm_load_text_domain' );
...
if(version_compare(phpversion(), '7.0', '<')){
	function mailpoet_bh_php_version_notice(){
		?>
		<div class="error">
			<p><?php _e('MailPoet plugin requires PHP version 7.0 or newer, Please upgrade your PHP.', 'bounce-handler-mailpoet'); ?></p>
		</div>
		<?php
	}
