...
class BounceDetect {
	...
	public function getBouncedEmailBySearch($html,$ignore_emails){
		...
		/*
		 *Check if the email belongs to the subscriber or not **Added in version 1.3.17 by k4mrul
		 * */
		foreach ($target as $key => $value) {
			try {
				$subscriber = \MailPoet\API\API::MP('v1')->getSubscriber($key); // $identifier can be either a subscriber ID or e-mail
				if($subscriber) {
					return $key;
				}
		...
		//Old/inefficient way to detect bounce
