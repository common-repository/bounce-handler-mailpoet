...
if(!class_exists('Mailpoet_Bounce_Handler')){
	...
	class Mailpoet_Bounce_Handler
	{
		...
		public function bounce_handler_page()
		{
            ...
            //$settings = new SettingsController();//old mailpoet settings object. Doesn't work since MailPoet version 3.39.1
            $settings = SettingsController::getInstance();  //New settings object. Fixed added by k4mrul
				...
				if(isset($_POST['data']['bounce']) && is_array($_POST['data']['bounce'])){ // Check bounce is set and array
					...
					if(empty($error)){
						...
						if($save == true){ // Data saved
                            ...
                            $settings = SettingsController::getInstance();//mailpoet settings
                            $mailpoet_conf_bounce_address = $settings->get('bounce.address');
